...
public class UserServiceImpl implements UserService {
    ...
    public void updateSimpleUser(String token, SimpleUpdateUserRequest updateUserRequest) {
        ...
        user.ifPresentOrElse(
                u -> {
                    ...
                    String username = Optional.ofNullable(updateUserRequest.getUsername()).orElse(u.getUsername());
                    String password = Optional.ofNullable(updateUserRequest.getPassword())
                            .map(passwordEncoder::encode).orElse(u.getPassword());
                    u.setEmail(email);
    ...
    public void addFollow(String followerToken, String username) {
        ...
        if (followedUser.getId().equals(followerId)) {
            throw new InvalidFollowException("Hey, self love is important, but you can't follow yourself here!");
        }

        if (followedUser.getFollowers().contains(followerId)) {
            throw new InvalidFollowException("Ops, I know you love her/him, but you already follow this user!");
        }
